...
/// extra `valaar_drain` per Valaar cell crossed. Planar neighbours never cross
/// layers.
...
mod tests {
    ...
    fn genome(diet: f32, eff: f32) -> Genome {
        Genome::from_array([0.5, eff, 0.0, diet, 0.9, 0.5, 0.5, 0.5, 0.5, 1.0, 0.0, 0.0])
    }
    ...
    fn moves_uphill_toward_richer_valaar() {
        ...
        pop.spawn(TraitOrganism::new(Genome::from_array([0.5, 1.0, 1.0, 0.0, 0.9, 0.5, 0.5, 0.5, 0.5, 1.0, 0.0, 0.0]), start, 5.0));
    ...
    fn at_local_max_it_stays_put() {
        ...
        let mut pop = Population::new();
        pop.spawn(TraitOrganism::new(Genome::from_array([0.5, 1.0, 1.0, 0.0, 0.9, 0.5, 0.5, 0.5, 0.5, 1.0, 0.0, 0.0]), peak, 5.0));
        let mut rng = Rng::new(1);
        move_organisms(&space, &field, &mut pop, &eco, &mut rng, None, None);
    ...
    fn does_not_step_onto_impassable_richer_neighbor() {
        ...
        pop.spawn(TraitOrganism::new(
            Genome::from_array([0.5, 1.0, 1.0, 0.0, 0.9, 0.5, 0.5, 0.5, 0.5, 1.0, 0.0, 0.0]),
            start,
            5.0,
        ));
        let mut rng = Rng::new(1);
    ...
    fn swimmer(swim: f32) -> Genome {
        Genome::from_array([0.5, 1.0, 1.0, 0.0, 0.9, 0.5, 0.5, 0.5, swim, 1.0, 0.0, 0.0])
    }
    ...
    fn predator(size: f32) -> Genome {
        Genome::from_array([size, 1.0, 0.0, 1.0, 0.9, 0.5, 0.5, 0.5, 0.5, 1.0, 0.0, 0.0])
    }
    fn prey(size: f32) -> Genome {
        Genome::from_array([size, 1.0, 0.0, 0.0, 0.9, 0.5, 0.5, 0.5, 0.5, 1.0, 0.0, 0.0])
    }
    ...
    fn starving_organism_does_not_reproduce() {
        ...
        let g = Genome::from_array([0.5, 1.0, 0.0, 0.0, 1.0, 0.5, 0.5, 0.5, 0.5, 1.0, 0.0, 0.0]);
    ...
    fn tol_genome(heat_tol: f32, drought_tol: f32) -> Genome {
        Genome::from_array([0.5, 1.0, 0.0, 0.0, 0.9, 0.5, heat_tol, drought_tol, 0.5, 1.0, 0.0, 0.0])
    }
